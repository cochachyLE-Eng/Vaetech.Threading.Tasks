...
namespace Vaetech.Threading.Tasks
{
    public class TaskLocal
    {
        ...
        #region Run - Func<Task<T>>
        public void Run<T>(Func<T> action, Action<ActionResult<T>> result)
            => Run<T, Exception>(action, result);
        public void Run<T, TException>(Func<T> action, Action<ActionResult<T>> result)
            where TException : Exception
        {
            try
            {                
