...
namespace Vaetech.Threading.Tasks
{
    ...
    {
        #region SplitAsync - Func<ListEvent<T, T1, T2, T3, T4, T5>, Task>[]
        public static async Task SplitAsync<T, T1, T2, T3, T4, T5>(List<T> data, T1 item1, T2 item2, T3 item3, T4 item4, T5 item5, params Func<ListEvent<T, T1, T2, T3, T4, T5>, Task>[] funcs)
            => await SplitAsync<T, T1, T2, T3, T4, T5>(ProcessType.Default, data, item1, item2, item3, item4, item5, funcs);
        public static async Task SplitAsync<T, T1, T2, T3, T4, T5>(ProcessType processType, List<T> data, T1 item1, T2 item2, T3 item3, T4 item4, T5 item5, params Func<ListEvent<T, T1, T2, T3, T4, T5>, Task>[] funcs)
        {
            if (!data.Any()) return;
            int i = 0, co = -1, lots = funcs.Count(), c = Count(data.Count, ref lots);

            List<Task> tasks = new List<Task>();
            foreach (Func<ListEvent<T, T1, T2, T3, T4, T5>, Task> fn in funcs.Take(c * lots))
            {
                int re = ++co == lots -1 ? data.Count % lots : 0;
                switch (processType)
                {
                    case ProcessType.Enqueue:
                        await fn.Invoke(new ListEvent<T, T1, T2, T3, T4, T5>(processType, data.GetRange(c * i++, c + re), item1, item2, item3, item4, item5, container: co));
                        break;
                    case ProcessType.RunAll:
                    default:
                        tasks.Add(fn(new ListEvent<T, T1, T2, T3, T4, T5>(processType, data.GetRange(c * i++, c + re), item1, item2, item3, item4, item5, container: co)));
                        break;
                }
            }
        ...
        public class ListEvent<T, T1, T2, T3, T4, T5>
        {
            ...
            public async Task EventAsync(params Func<ListEventHandler<T, T1, T2, T3, T4, T5>>[] events)
            {
                ...
                List<Task> tasks = new List<Task>();
                foreach (Func<ListEventHandler<T, T1, T2, T3, T4, T5>> fn in events.Take(c * lots))
                {
                    ListEventHandler<T, T1, T2, T3, T4, T5> handler = fn.Invoke();
