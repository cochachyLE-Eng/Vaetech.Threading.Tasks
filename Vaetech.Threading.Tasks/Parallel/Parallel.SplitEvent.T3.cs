            ...
            foreach (Func<ListEvent<T, T1, T2, T3>, Task> action in actions)
            {
                ...
                switch (typeProcess)
                {
                    case ProcessType.Enqueue:
                        await action.Invoke(new ListEvent<T, T1, T2, T3>(typeProcess, data.GetRange(c * i++, c + re), item1, item2, item3, container: co - 1));
                        break;
                    case ProcessType.RunAll:
                    default:
                        tasks.Add(action(new ListEvent<T, T1, T2, T3>(typeProcess, data.GetRange(c * i++, c + re), item1, item2, item3, container: co - 1)));
                        break;
                }
            }
