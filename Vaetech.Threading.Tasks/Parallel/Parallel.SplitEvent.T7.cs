...
namespace Vaetech.Threading.Tasks
{
    ...
    {
        #region WorkerAsync - Func<ListEvent<T, T1, T2, T3, T4, T5, T6, T7>, Task>[]
        public static async Task SplitEventAsync<T, T1, T2, T3, T4, T5, T6, T7>(List<T> data, T1 item1, T2 item2, T3 item3, T4 item4, T5 item5, T6 item6, T7 item7, params Func<ListEvent<T, T1, T2, T3, T4, T5, T6, T7>, Task>[] actions)
            => await SplitEventAsync<T, T1, T2, T3, T4, T5, T6, T7>(ProcessType.Default, data, item1, item2, item3, item4, item5, item6, item7, actions);
        public static async Task SplitEventAsync<T, T1, T2, T3, T4, T5, T6, T7>(ProcessType typeProcess, List<T> data, T1 item1, T2 item2, T3 item3, T4 item4, T5 item5, T6 item6, T7 item7, params Func<ListEvent<T, T1, T2, T3, T4, T5, T6, T7>, Task>[] actions)
        {
            if (!data.Any()) return;
            int i = 0, co = 0, lots = actions.Count(), c = count(data.Count, ref lots);

            List<Task> tasks = new List<Task>();
            foreach (Func<ListEvent<T, T1, T2, T3, T4, T5, T6, T7>, Task> action in actions)
            {
                int re = (++co == actions.Count() ? data.Count % lots : 0);
                switch (typeProcess)
                {
                    case ProcessType.Enqueue:
                        await action.Invoke(new ListEvent<T, T1, T2, T3, T4, T5, T6, T7>(typeProcess, data.GetRange(c * i++, c + re), item1, item2, item3, item4, item5, item6, item7, container: co - 1));
                        break;
                    case ProcessType.RunAll:
                    default:
                        tasks.Add(action(new ListEvent<T, T1, T2, T3, T4, T5, T6, T7>(typeProcess, data.GetRange(c * i++, c + re), item1, item2, item3, item4, item5, item6, item7, container: co - 1)));
                        break;
                }
            }
        ...
        public class ListEvent<T, T1, T2, T3, T4, T5, T6, T7>
        {
            ...
            public async Task EventAsync(params Func<ListEventHandler<T, T1, T2, T3, T4, T5, T6, T7>>[] events)
            {
                ...
                foreach (Func<ListEventHandler<T, T1, T2, T3, T4, T5, T6, T7>> ev in events)
                {
                    ListEventHandler<T, T1, T2, T3, T4, T5, T6, T7> handler = ev.Invoke();
